...
//			require_once  'modulos/Discipulo/visao/listar.php';
    ...
    public function novo($url)
    {
        if (empty( $url['post'])) {
            ...
            require_once 'modulos/Governo/visao/tipoRede/novo.php';

        } else {
            $governo = new \Governo\Modelo\Governo();

            $post = $url['post'] ;
            $governo->nome = $post['nome'];

            if (!$governo->salvar()) {
                $governo->atualizar();
            }

            header ('location:/governo/governo/listar');
        }
    ...
    {
        $governo = new \Governo\Modelo\Governo();
        $governos = $governo->listarTodos();
        require_once 'modulos/Governo/visao/tipoRede/listar.php';
    }
...
public function listarMembrosRede($url)
{
    ...
    public function atualizar($url)
    {
        if ( empty ( $url['post'] ) ) {
            ...
            $governo = new \Governo\Modelo\Governo();
            $governo->id = $id;
            $governo = $governo->listarUm($id);

            require_once 'Governo/visao/tipoRede/atualizar.php';
